...
test('does not invent employer premiums', () => {
  const result = forecastShiftPay({
    start: '2026-09-06T07:00:00+01:00',
    end: '2026-09-06T15:00:00+01:00',
    unpaidBreakMinutes: 60,
    baseRatePence: 1790,
  });
  assert.equal(result.grossPence, 12530);
...
test('applies only explicitly supplied premium segments', () => {
  const result = forecastShiftPay({
    ...
    unpaidBreakMinutes: 60,
    baseRatePence: 1790,
    premiumSegments: [{ minutes: 420, multiplierBps: 16000 }],
  });
...
test('handles explicit overnight dated datetimes', () => {
  const result = forecastShiftPay({
    start: '2026-09-01T22:00:00+01:00',
    end: '2026-09-02T06:00:00+01:00',
